...
import com.example.mtg.Repository.CardPurchaseAssociationRepository;
...
@Controller @RequestMapping(path = "/inventory") public class InventoryController {
	...
	void setCount(@PathVariable("setName") String setName) {
		Set<Card> cards =
				cardRepository.findAllBySet(setName).stream().filter(c -> c.getRarity().equals(Rarity.RARE) || c.getRarity().equals(Rarity.MYTHIC)).collect(
				Collectors.toSet());
	...
	void stock() {
		...
		List<Transaction> transactions = associations.stream().map(a -> a.getTransaction())
				.collect(Collectors.toList());
		Transaction min = transactions.stream().min((a, b) -> a.getDate().compareTo(b.getDate())).orElseThrow(() -> new ResourceNotFoundException());
		double sum =0;
...
//					System.out.println(date.toString()+"\t" + cpa.getCard().getId() +"\t" + priceToAdd+"\t"+cpa.getCard().getName()+"\t"+cpa.isFoil());
				}
			}
	...
	public @ResponseBody void delteAssociations (){
	...
	String getInput(@PathVariable("setName") String setName) {
		...
		{
			ans += card.getId() +"\t" + card.getName() +"\t" + card.getCollector_number() + "\t" + card.getManaCost() +
				"\t" + card.getRarity()+"\n";
		}
		for(Card card: cards.stream().filter(c -> c.getRarity().equals(Rarity.UNCOMMON)).collect(Collectors.toList()))
		{
			ans += card.getId() +"\t" + card.getName() +"\t" + card.getCollector_number() + "\t" + card.getManaCost() +
					"\t" + card.getRarity()+"\n";
		}
		for(Card card:
				cards.stream().filter(c -> c.getRarity().equals(Rarity.RARE) || c.getRarity().equals(Rarity.MYTHIC)).collect(Collectors.toList()))
		{
			ans += card.getId() +"\t" + card.getName() +"\t" + card.getCollector_number() + "\t" + card.getManaCost() +
					"\t" + card.getRarity()+"\n";
		}
	...
	public @ResponseBody String mtgGoldfishAll()
	{
		String ans ="";
		...
		Date yesteday = addDays(today,-1);
		Date weekago = addDays(today,-7);
		for(String key: cpas.keySet()) {
			...
			if(foils>0) {
				...
				ans += card.getRarity()+"\t";
				ans += foils +"\t";
				Price todaysPrice = prices.stream().filter(p -> p.getDate().toString().equals(today.toString())).findFirst().orElse(null);
				Price yestedaysPrice = prices.stream().filter(p -> p.getDate().toString().equals(yesteday.toString())).findFirst().orElse(null);
				Price weekagoPrice = prices.stream().filter(p -> p.getDate().toString().equals(weekago.toString())).findFirst().orElse(null);
				if(todaysPrice!=null)
				{
		...
		List<String> names = cardPurchaseAssociationRepository.findAll().stream().map(c -> c.getCard().getName())
	...
	public @ResponseBody String mtgGoldfish()
	{
		String ans="";
	...
	String temp() {
		...
		List<CardPurchaseAssociation> temp = cardPurchaseAssociationRepository.findAll().stream()
				.filter(c -> c.getCard().getRarity().equals(Rarity.COMMON) || c.getCard().getRarity().equals(Rarity.UNCOMMON)).collect(Collectors.toList());
